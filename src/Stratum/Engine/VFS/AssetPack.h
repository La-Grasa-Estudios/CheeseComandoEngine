...
public:
    ...
    static AssetPack* MountFile(std::string pack);
    static void ExtractPakFile(std::string pack, int fileIndex = 0);

    std::stringstream GetFile(AssetFileEntry& entry);
...
struct AssetPacker {
    ...
    void Pack(std::string directory, std::string out, bool dirIsRoot, const char** pIgnore = NULL, int32_t nbIgnore = 0);
