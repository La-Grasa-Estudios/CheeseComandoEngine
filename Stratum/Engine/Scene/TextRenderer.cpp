...
TextRenderer::TextRenderer(SceneResources* pResources)
{
...
void TextRenderer::DrawSprite(const SpriteInstance& instance)
{
	auto spriteTransform = glm::identity<glm::mat4>();

	spriteTransform = glm::scale(spriteTransform, glm::vec3(glm::vec2(instance.RenderSize), 1.0f));
...
void TextRenderer::Render(Render::GraphicsCommandBuffer* pCmdBuffer)
{
...
void TextRenderer::End(Render::CopyCommandBuffer* pCmdBuffer)
{
	if (mRenderQueue.empty()) // No need to render if there isn't anything in the queue
		return;
...
void TextRenderer::SetResources(SceneResources* pRsc)
{
