...
bool ShaderCompiler::build_object(const char* input, const char* output, shader_type type, int nbPermutations)
{
	...
	std::filesystem::file_time_type lastWrite = std::filesystem::last_write_time(std::string("Data/").append(input));
	...
	JobManager::Dispatch(sourcePreprocessor.GetPermutationCount(), 1, [&](JobDispatchArgs args) {
		...
		if (type == shader_type::fragment || type == shader_type::vertex)
		{
			...
			for (int k = 0; k < 2; k++)
			{
				...
				std::vector<shaderbinding_t> shaderBindingsVk;
				...
				std::vector<uint8_t> targetVK = GetShaderBinary(ss, sourcePreprocessor1, input, type, 0, sourcePreprocessor.GetPermutation(i), shaderBindingsVk, Render::RendererAPI::VULKAN);
