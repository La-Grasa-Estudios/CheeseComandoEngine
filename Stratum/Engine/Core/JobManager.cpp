...
namespace JobSystem
{
    ...
    bool isSerial = false; // Used for OpenGL compatibility
...
void JobManager::Dispatch(uint32_t jobCount, uint32_t groupSize, const std::function<void(JobDispatchArgs)>& job, size_t sharedMemorySize)
{
    ...
    const uint64_t groupCount = (jobCount + groupSize - 1) / groupSize;

    // The main thread label state is updated:
    currentLabel += groupCount;

    for (uint64_t groupIndex = 0; groupIndex < groupCount; ++groupIndex)
    {
        // For each group, generate one real job:
        auto jobGroup = [jobCount, groupSize, job, groupIndex, sharedMemorySize]() {

            // Calculate the current group's offset into the jobs:
            const uint64_t groupJobOffset = groupIndex * groupSize;
