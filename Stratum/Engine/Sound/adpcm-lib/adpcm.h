...
struct ADPCMHeader
{
    ...
    long  chunkSize = 0;                        // 16 + (8 + dataSize)

    long channelCount = 0;                      // Number of channels in the audio
    long  sampleCount = 0;                      // Number of samples in the data
    
    sample ch1KeySample = 0;                    // Key Sample for channel 1
    short  ch1StepIndex = 0;                    // Step Table starting index for channel 1
    sample ch2KeySample = 0;                    // Key Sample for channel 2
    short  ch2StepIndex = 0;                    // Step Table starting index for channel 2

    char  dataID[4] = {'d','a','t','a'};    // Contains the letters "data" in ASCII form
    long  dataSize = 0;                         // This is the number of bytes in the data
};
